...
async function main() {
  ...
  for (const inv of invitaciones) {
    ...
    if (config.galeriaFotos && Array.isArray(config.galeriaFotos)) {
      ...
      config.galeriaFotos = config.galeriaFotos.filter(f => typeof f === 'string' && !f.startsWith('data:image'));
    ...
    // Limpiar Portada
    if (config.coverUrl && typeof config.coverUrl === 'string' && config.coverUrl.startsWith('data:image')) {
