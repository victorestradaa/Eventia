...
import {
  TrendingUp, ShoppingCart, DollarSign, Receipt, Loader2, Calendar, AlertCircle, Download, Package,
  Banknote, CreditCard, ArrowLeftRight, MoreHorizontal, Clock, CheckCircle2, Sparkles, Truck,
  TrendingDown, Minus, Trophy, Boxes, Tag,
} from 'lucide-react';
...
const COLORES_ESTADO: Record<string, string> = {
  PENDIENTE: '#f59e0b', EN_PREPARACION: '#3b82f6', LISTO: '#8b5cf6', ENTREGADO: '#10b981',
};

const COLORES_METODO: Record<string, string> = {
  EFECTIVO: '#10b981', TARJETA: '#3b82f6', TRANSFERENCIA: '#8b5cf6', OTRO: '#6b7280',
};

const ICONOS_METODO: Record<string, any> = {
  EFECTIVO: Banknote, TARJETA: CreditCard, TRANSFERENCIA: ArrowLeftRight, OTRO: MoreHorizontal,
};

const ICONOS_ESTADO: Record<string, any> = {
  PENDIENTE: Clock, EN_PREPARACION: Package, LISTO: Sparkles, ENTREGADO: Truck,
};

const LABELS_ESTADO: Record<string, string> = {
  PENDIENTE: 'Pendientes', EN_PREPARACION: 'En preparación', LISTO: 'Listos', ENTREGADO: 'Entregados',
};

const PALETA_CAT = ['#d4af37', '#3b82f6', '#10b981', '#8b5cf6', '#f59e0b', '#ef4444', '#06b6d4', '#ec4899'];
...
export default function ReportesPVClient({ proveedorId, reporteInicial }: Props) {
  ...
  const aplicarRango = async (rangoId: string, customDesde?: string, customHasta?: string) => {
    ...
    if (rangoId === 'HOY') {
      ...
      fin.setDate(fin.getDate() - 1); fin.setHours(23, 59, 59, 999);
    } else if (rangoId === '7D') {
      inicio.setDate(inicio.getDate() - 6); inicio.setHours(0, 0, 0, 0);
    } else if (rangoId === '30D') {
      inicio.setDate(inicio.getDate() - 29); inicio.setHours(0, 0, 0, 0);
    ...
    setCargando(true);
    const res = await getReportePV(proveedorId, inicio.toISOString().slice(0, 10), fin.toISOString().slice(0, 10));
    setCargando(false);
  ...
  const horaPico = useMemo(() => {
    ...
    const max = reporte.porHora.reduce((m, h) => (h.ventas > m.ventas ? h : m), reporte.porHora[0]);
    return max && max.ventas > 0 ? max : null;
  }, [reporte]);
  ...
  const exportarCSV = () => {
    ...
    rows.push(`Unidades vendidas,${reporte.unidadesVendidas}`);
    rows.push(`Ticket promedio,${reporte.ticketPromedio.toFixed(2)}`);
    if (reporte.margenEstimado != null) rows.push(`Margen estimado,${reporte.margenEstimado.toFixed(2)}`);
    ...
    rows.push('');
    rows.push('POR HORA');
    rows.push('Hora,Ventas,Pedidos');
    reporte.porHora.forEach((h) => rows.push(`${h.hora},${h.ventas.toFixed(2)},${h.pedidos}`));
    rows.push('');
    rows.push('CATEGORIAS');
    rows.push('Categoria,Cantidad,Total');
    reporte.porCategoria.forEach((c) => rows.push(`"${c.categoria.replace(/"/g, '""')}",${c.cantidad},${c.total.toFixed(2)}`));
    rows.push('');
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {RANGOS.map((r) => (
          <button
            ...
            className={cn(
              ...
              rango === r.id ? 'bg-[#d4af37] text-black' : 'bg-[var(--color-fondo-card)] text-[var(--color-texto-suave)] hover:bg-[var(--color-fondo-hover)]'
            )}
          >
            {r.label}
          </button>
        ...
        {reporte && (
          <button onClick={exportarCSV} className="ml-auto inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold border border-[var(--color-borde-suave)] hover:bg-[var(--color-fondo-hover)]">
      ...
      {rango === 'CUSTOM' && (
        <div className="flex flex-wrap gap-2 items-end p-4 rounded-2xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)]">
          ...
          <button onClick={aplicarCustom} disabled={cargando} className="px-5 py-2 rounded-xl bg-[#d4af37] text-black font-black uppercase text-xs tracking-widest disabled:opacity-50 inline-flex items-center gap-2">
      ...
      {reporte && (
        <div className={cn('space-y-5 transition-opacity', cargando && 'opacity-60')}>
          ...
          <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <KPI icon={TrendingUp}   label="Ventas"          valor={formatearMoneda(reporte.totalVentas)} delta={reporte.deltaVentas} accent />
            <KPI icon={ShoppingCart} label="Pedidos"         valor={String(reporte.numPedidos)}           delta={reporte.deltaPedidos} />
            <KPI icon={Receipt}      label="Ticket promedio" valor={formatearMoneda(reporte.ticketPromedio)} delta={reporte.deltaTicket} />
            <KPI icon={Boxes}        label="Unidades"        valor={String(reporte.unidadesVendidas)} />
          </section>

          {/* KPIs secundarios */}
          <section className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <MiniKPI icon={DollarSign} label="Cobrado"   valor={formatearMoneda(reporte.totalCobrado)}  tono="emerald" />
            <MiniKPI icon={DollarSign} label="Pendiente" valor={formatearMoneda(reporte.totalPendiente)} tono="rose" />
            {reporte.margenEstimado != null && (
              <MiniKPI icon={TrendingUp} label="Margen estimado" valor={formatearMoneda(reporte.margenEstimado)} tono="emerald" />
            )}
            {horaPico && (
              <MiniKPI icon={Clock} label="Hora pico" valor={`${String(horaPico.hora).padStart(2, '0')}:00`} hint={formatearMoneda(horaPico.ventas)} />
            )}
            {reporte.mejorDia && (
              <MiniKPI
                icon={Trophy}
                label="Mejor día"
                valor={new Date(reporte.mejorDia.fecha + 'T12:00:00').toLocaleDateString('es-MX', { day: '2-digit', month: 'short' })}
                hint={formatearMoneda(reporte.mejorDia.ventas)}
                tono="amber"
              />
            )}
          </section>

          {/* Gráficas: ventas por día + métodos */}
          <section className="grid lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 rounded-2xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)] p-5">
              ...
              {dataPorDia.length === 0 ? (
                <div className="h-64 flex items-center justify-center text-[var(--color-texto-muted)] text-sm">Sin ventas en este periodo.</div>
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={dataPorDia} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip
                      formatter={(value: any) => [formatearMoneda(Number(value)), 'Ventas']}
            ...
            <div className="rounded-2xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)] p-5">
              ...
              {dataPorMetodo.length === 0 ? (
                <div className="h-44 flex items-center justify-center text-[var(--color-texto-muted)] text-sm">Sin cobranza.</div>
              ) : (
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    <Pie data={dataPorMetodo} dataKey="value" nameKey="name" innerRadius={45} outerRadius={70} paddingAngle={2}>
                      {dataPorMetodo.map((d, i) => <Cell key={i} fill={COLORES_METODO[d.name] || '#999'} />)}
                    </Pie>
          ...
          {/* Hora pico + Categorías */}
          <section className="grid lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 rounded-2xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)] p-5">
              <h3 className="text-sm font-black uppercase tracking-widest text-[var(--color-texto)] mb-4 flex items-center gap-2">
                <Clock size={14} /> Distribución por hora
              </h3>
              {dataPorHora.every((h) => h.ventas === 0) ? (
                <div className="h-44 flex items-center justify-center text-[var(--color-texto-muted)] text-sm">Sin actividad.</div>
              ) : (
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={dataPorHora} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--color-borde-suave)" />
                    <XAxis dataKey="hora" tick={{ fontSize: 9, fill: 'var(--color-texto-muted)' }} interval={2} />
                    <YAxis tick={{ fontSize: 10, fill: 'var(--color-texto-muted)' }} tickFormatter={(v) => `$${v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v}`} />
                    <Tooltip
                      formatter={(value: any) => [formatearMoneda(Number(value)), 'Ventas']}
                      contentStyle={{ background: 'var(--color-fondo-card)', border: '1px solid var(--color-borde-suave)', borderRadius: 12, fontSize: 12 }}
            ...
            <div className="rounded-2xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)] p-5">
              <h3 className="text-sm font-black uppercase tracking-widest text-[var(--color-texto)] mb-4 flex items-center gap-2">
                <Tag size={14} /> Por categoría
              </h3>
              {reporte.porCategoria.length === 0 ? (
                <div className="h-44 flex items-center justify-center text-[var(--color-texto-muted)] text-sm">Sin datos.</div>
              ) : (
                <div className="space-y-2.5">
                  {reporte.porCategoria.map((c, i) => {
                    const max = reporte.porCategoria[0].total || 1;
                    const pct = (c.total / max) * 100;
                    return (
                      <div key={c.categoria}>
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="font-bold truncate" style={{ color: PALETA_CAT[i % PALETA_CAT.length] }}>{c.categoria}</span>
                          <span className="font-black">{formatearMoneda(c.total)}</span>
                        </div>
                        <div className="h-1.5 bg-[var(--color-fondo-hover)] rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: PALETA_CAT[i % PALETA_CAT.length] }} />
                        </div>
                        <p className="text-[9px] text-[var(--color-texto-muted)] mt-0.5">{c.cantidad} unidades</p>
                      </div>
                    );
                  })}
                </div>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
            <div className="rounded-2xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)] p-5">
              <h3 className="text-sm font-black uppercase tracking-widest text-[var(--color-texto)] mb-4">Pedidos por estado</h3>
            ...
            <div className="rounded-2xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)] p-5 space-y-3">
              <h3 className="text-sm font-black uppercase tracking-widest text-[var(--color-texto)]">Resumen</h3>
                  ...
                  return (
                    <div key={i} className="px-5 py-3">
                      ...
                      <div className="w-full h-1.5 bg-[var(--color-fondo-hover)] rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-[#d4af37] to-[#b89547]" style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  );
                })}
...
function KPI({ icon: Icon, label, valor, delta, accent = false }: {
  icon: any; label: string; valor: string; delta?: number | null; accent?: boolean;
}) {
  const showDelta = delta != null && isFinite(delta);
  const positivo = showDelta && delta! > 0;
  const negativo = showDelta && delta! < 0;
  const DeltaIcon = positivo ? TrendingUp : negativo ? TrendingDown : Minus;
  const deltaColor = positivo ? 'text-emerald-600' : negativo ? 'text-rose-600' : 'text-[var(--color-texto-muted)]';
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {showDelta && (
          <span className={cn('inline-flex items-center gap-0.5 text-[10px] font-black px-1.5 py-0.5 rounded-full', deltaColor, positivo ? 'bg-emerald-500/10' : negativo ? 'bg-rose-500/10' : 'bg-[var(--color-fondo-hover)]')}>
            <DeltaIcon size={9} />{Math.abs(delta!).toFixed(0)}%
          </span>
        )}
      </div>
      <p className={cn('text-[10px] font-black uppercase tracking-widest', accent ? 'text-[#1F2937]/70' : 'text-[var(--color-texto-muted)]')}>{label}</p>
      <p className={cn('text-xl font-black mt-0.5 tracking-tight', accent ? 'text-[#1F2937]' : 'text-[var(--color-texto)]')}>{valor}</p>
    </div>
  );
}

function MiniKPI({ icon: Icon, label, valor, hint, tono }: {
  icon: any; label: string; valor: string; hint?: string; tono?: string;
}) {
  ...
  const color = tono ? colorMap[tono] : 'text-[#d4af37] bg-[#d4af37]/10';

  return (
    <div className="rounded-xl border border-[var(--color-borde-suave)] bg-[var(--color-fondo-card)] p-3">
      <div className="flex items-center gap-2 mb-1">
        <div className={cn('w-6 h-6 rounded-md flex items-center justify-center', color)}>
          <Icon size={11} />
        </div>
        <p className="text-[9px] font-black uppercase tracking-widest text-[var(--color-texto-muted)]">{label}</p>
      </div>
      <p className="text-base font-black text-[var(--color-texto)]">{valor}</p>
      {hint && <p className="text-[10px] text-[var(--color-texto-muted)]">{hint}</p>}
    </div>
  );
}
...
function Row({ label, value, tono }: { label: string; value: number; tono?: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={cn('text-base font-black', tono === 'rose' ? 'text-rose-500' : 'text-[var(--color-texto)]')}>{value}</span>
    </div>
  );
}
